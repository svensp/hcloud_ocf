...
from ..pacemaker.ocf import ReturnCodes, AbortWithError, ResourceAgent, Parameter
from ..pacemaker.validators import IpvAnyValidator, StringLengthValidator
from ..hcloud.hostfinder import makeHostFinder
import time
...
class FloatingIp(ResourceAgent):
    def __init__(self, ipFinder = IpFinder()):
        ...
        ResourceAgent.__init__(self, 'floating_ip', '0.1.0', 'Manage Hetzner Cloud Floating Ips',
                '''
        ...
        self.floatingIp = Parameter('ip', shortDescription='Hetner Cloud Ip-Address x.x.x.x' ,
                description='''
        ...
        self.floatingIp.validate = IpvAnyValidator(self.floatingIp)
        ...
        self.failOnHostfindFailure = Parameter('fail_on_host_find_failure', shortDescription='Exit with misconfigured if the host was not found' ,
                ...
                ''',
                required=False, unique=False)
        self.sleep = Parameter('sleep', default='5', shortDescription='Sleep duration when an api request fails' ,
                description='''
        ...
        while not success:
            try:
                ...
                if self.failOnHostfindFailure.get() is 'true':
                    raise AbortWithError(ReturnCodes.isMissconfigured, 'Failed to find server')
                time.sleep( self.wait )
    ...
    def findIp(self):
        ...
        while not success:
            try:
                ...
                # Ip not found in api
                if self.failOnHostfindFailure.get() is 'true':
                    raise AbortWithError(ReturnCodes.isMissconfigured, 'Failed to find ip')
                time.sleep( self.wait )
